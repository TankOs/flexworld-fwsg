...
int main() {
	...
	wireframe_static_mesh->set_local_transform( sg::Transform( sf::Vector3f( -0.3f, -0.3f, 0 ) ) );
		...
		while( window.pollEvent( event ) ) {
			if( event.type == sf::Event::Closed ) {
				...
				if( event.key.code == sf::Keyboard::Escape ) {
					...
					const sg::DepthTestState* depth_test_state = root_node->find_state<sg::DepthTestState>();

					root_node->set_state(
						sg::DepthTestState(
							(depth_test_state != nullptr) ? !depth_test_state->is_set() : true
						)
					);
				}
			}
